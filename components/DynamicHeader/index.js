...
class DynamicHeader extends React.Component {
  constructor(props) {
    ...
    if (
      ...
      console.log(
        "WARNING [react-sticky-dynamic-header]: more than 2 components were passed as children, only the first 2 are used, the rest will be ignored!"
      );
      if (!this.props.children[0].type || !this.props.children[1].type) {
        componentOK = false;
      }
    } else if (!this.props.children[0].type || !this.props.children[1].type)
      componentOK = false;
  ...
  _calcTranslation(h1, h2) {
    ...
    if (heightDiff <= 0 || !this.props.useHeadersDifference)
      heightDiff = parseInt(h1);
  ...
  render() {
      ...
      tmpBigComponent = (
        ...
        >
          <div style={{ paddingTop: "15px" }}>
            ERROR [react-sticky-dynamic-header]: 2 components must be passed as
            children!
          </div>
        </div>
      );
    ...
    if (this.state.headerType == "big") {
      ...
      if (componentOK) {
        bigHeaderComponent = React.cloneElement(
          this.props.children[this.state.bigIndex],
          { key: elementName + "_1" }
        );
      }
      returnEle = (
        <div>
          ...
          <ReactCSSTransitionGroup
            transitionName={elementName + "_1"}
            transitionAppear={false}
            transitionEnterTimeout={500}
            transitionLeaveTimeout={500}
            transitionLeave={false}
            transitionEnter={this.props.hasEffect}
          >
            {bigHeaderComponent}
          </ReactCSSTransitionGroup>
        </div>
      ...
      if (componentOK) {
        smallHeaderComponent = React.cloneElement(
          this.props.children[this.state.smallIndex],
          { key: elementName + "_2" }
        );
      }
      returnEle = (
        <div>
          ...
          <div
            style={{
              height: this.state.bigElementHeight + "px",
              width: "100%",
              clear: "both"
            }}
          />
